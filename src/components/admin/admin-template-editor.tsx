...
export function AdminTemplateEditor({ templates }: { templates: Template[] }) {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = editingTemplate
        ? await updateGlobalTemplate(editingTemplate.id, fd)
        : await createGlobalTemplate(fd);
  ...
  const handleDelete = (id: string) => {
    if (!confirm("Hapus template global ini? Organisasi yang belum membuat template sendiri akan kehilangan akses ke template ini.")) return;
    startTransition(async () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <p className="text-sm text-muted-foreground">
          {templates.length} template global tersimpan
        </p>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Card key={t.id}>
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground mb-2">
                Digunakan sebagai fallback jika organisasi belum memiliki template sendiri.
              </p>
        ...
        {templates.length === 0 && (
          <div className="col-span-full text-center py-12 text-muted-foreground">
            ...
            <p>Belum ada template global.</p>
            <p className="text-sm mt-1">Template global digunakan sebagai fallback saat organisasi belum memiliki template sendiri.</p>
          </div>
        )}
      </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingTemplate ? "Edit Template Global" : "Buat Template Global Baru"}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Contoh: Sertifikat Tera Default"
                />
              </div>
              <div className="space-y-2">
